...
class Ion_auth_model extends CI_Model
{
	...
	public function change_user_picture($image){
	...
	**/
	...
	public function create_category($category_name = FALSE, $additional_data = array())
	{
		// bail if the group name was not passed
		if(!$category_name)
		{
			$this->set_error('category_name_required');
			return FALSE;
		}

		// bail if the group name already exists
		$existing_group = $this->db->get_where($this->tables['categories'], array('name' => $category_name))->num_rows();
		if($existing_group !== 0)
		{
		...
		//filter out any data passed that doesnt have a matching column in the groups table
		//and merge the set group data and the additional data
		if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['categories'], $additional_data), $data);

		$this->trigger_events('extra_category_set');

		// insert the new group
		$this->db->insert($this->tables['categories'], $data);
	...
	public function view_categories_and_subcategories(){
